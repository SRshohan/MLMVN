...
function [hidneur_weights, outneur_weights, iterations] = Net_learn_testcodeC(input, hidneur_weights, hidneur_num, outneur_weights, outneur_num, sec_nums, RMSE_thresh, local_thresh)
...
% hidneur_weights -     Matrix of initial weights of all hidden neurons
% hidneur_num - # of hidden neurons in a single hidden layer
% outneur_weights -     Matrix of initial weights for the output neurons
...
% if no starting weight are provided, then random weights should be
% generated
if ~isempty(hidneur_weights)
...
%Shift all desired phase values by half-sector counter-clockwise to move
%them to the bisector of the desired sector
phase_d = phase_d + sec_size(pp)/2;
%for pp = 1 : outneur_num
%    
...
% initial value of the min # of errors is set to the number of samples in
% the learning set
min_nesovpad = N;
...
% initialization of LearninFlag. If LearningFlag = 1, we have to learn
LearnFlag = 1;

%initialization of variables for storing errors (just assigning some large
%values)
min_err_all = 1000.0;
min_RMSE = 1000.0;

% N_x_outneur_num = (# of learning samples (N) ) x (# of output neurons)
...
%    end
    ...
    % intialization of current_labels - an array containing labels, which
    % were assigned as final network outputs
    % current_labels = zeros(N, outneur_num);
    % calculation of current_labels, that is network outputs 
    ...
    % nesovpad = 0;
    % initialization of angular RMSE
    ang_RMSE = 0;
    
    % calculation of errors in terms of
    %label differences per each learning sample
    
    diff_labels = sum(abs(current_labels - y_d), 2);
    
    % nesovpad = # of errors over the learning set
    ...
    % ang_RMSE here is a sum of squared angular errors (for all output
    ...
    % and # of samples in the learning set
    ang_RMSE = sqrt(mean(ang_RMSE) / N_x_outneur_num);       
